--  CYBER DEVELOPER -- 
--  https://discord.com/invite/yjVZyWtdFJ -- 
--  Cyber#3138 -- 
...
AddEventHandler('gcPhone:yellow_postIlan', function(message, image)
    ...
    for _, i in ipairs(GetPlayers()) do
   	 TriggerClientEvent('co_notify:client:SendNotifys', i, { app = "yellow" , title = "@"..maven_datastore[1].username, content = message })
    end
    YellowPagesPostIlan(message, image, sourcePlayer, srcIdentifier)
end)

 function sendDiscordYellow(post)
    local discord_webhook = Config.YellowWebbook --YellowPages Bot
    ...
    local data = {
        ["username"] = "Seccion Amarilla - Nueva Publicacion de: " .. post.author,
        ["embeds"] = {{
			["footer"] = {
